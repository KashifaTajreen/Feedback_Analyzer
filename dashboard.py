...
def run_dashboard(parent, user_email):
    ...
    root.mainloop()
# import tkinter as tk
# from data import get_all_sessions, init_sessions_db 
# from host_tools import open_create_session, verify_host_password
# from gui import run_gui 
# from analysis_view import run_analysis_view
...
#     BTN_PRIMARY = "#0ea5e9"  
#     BTN_SECONDARY = "#334155"
#     GIVE_FEEDBACK_GREEN = "#10b981" # Your original emerald green
...
#     add_btn = tk.Button(root, text="+ Create New Session", 
#                         bg=BTN_PRIMARY, fg="white",
#                         activebackground=ACCENT, activeforeground="black",
#                         font=("Segoe UI", 11, "bold"), relief="flat",
#                         height=2, cursor="hand2", padx=20,
#                         command=lambda: open_create_session(root, load_sessions))
#     add_btn.place(relx=0.95, rely=0.92, anchor="se")

#     main_frame = tk.Frame(root, bg=BG_DARK)
#     main_frame.pack(fill="both", expand=True)

#     header_frame = tk.Frame(main_frame, bg=BG_DARK)
#     header_frame.pack(pady=(50, 20), fill="x")

#     tk.Label(header_frame, text="FEEDBACK ANALYZER",
#              font=("Segoe UI", 32, "bold"),
#              bg=BG_DARK, fg=ACCENT).pack()

#     tk.Label(header_frame, text=f"Logged in : {user_email}",
#              font=("Segoe UI", 14),
...
#                 # Added highlightthickness=1 to enable the border glow on hover
#                 card = tk.Frame(scrollable_frame, bg=BG_CARD, width=300, height=260, padx=20, pady=15,
#                                 highlightbackground=BTN_SECONDARY, highlightthickness=1)
#                 card.grid(row=i//3, column=i%3, padx=20, pady=20)
#                 card.pack_propagate(False)

#                 tk.Label(card, text=s['name'], font=("Segoe UI", 16, "bold"), bg=BG_CARD, fg=TEXT_MAIN).pack(pady=(5, 2))
#                 tk.Label(card, text=f"Host: {s['host']}", fg=ACCENT, bg=BG_CARD, font=("Segoe UI", 10)).pack()
#                 tk.Label(card, text=f"ID: {s['id']}", fg=TEXT_DIM, bg=BG_CARD, font=("Segoe UI", 9)).pack()
#                 tk.Label(card, text=f"Date: {s['date']}", fg=ACCENT, bg=BG_CARD, font=("Segoe UI", 9)).pack()

#                 # Buttons
#                 btn_f = tk.Button(card, text="Give Feedback", bg=GIVE_FEEDBACK_GREEN, fg="white", relief="flat",
#                                   activebackground="#059669", font=("Segoe UI", 10, "bold"),
#                                   cursor="hand2", command=lambda session=s: run_gui(root, session))
#                 btn_f.pack(fill="x", pady=(20, 8), ipady=5)

#                 btn_v = tk.Button(card, text="View Analysis", bg=BTN_SECONDARY, fg=TEXT_MAIN, relief="flat",
#                                   activebackground="#475569", font=("Segoe UI", 10, "bold"),
#                                   cursor="hand2", command=lambda session=s: verify_host_password(
#                                       root, session, lambda: run_analysis_view(root, session)))
#                 btn_v.pack(fill="x", ipady=5)

#                 # --- HOVER EFFECTS ADDED TO OLD UI ---
#                 def on_enter(e, c=card):
#                     # Lift the background color and add a Cyan border glow
#                     c.config(bg="#2d3a4f", highlightbackground=ACCENT)
#                     for widget in c.winfo_children():
#                         if isinstance(widget, tk.Label):
#                             widget.config(bg="#2d3a4f")

#                 def on_leave(e, c=card):
#                     # Return to original card color and dim border
#                     c.config(bg=BG_CARD, highlightbackground=BTN_SECONDARY)
#                     for widget in c.winfo_children():
#                         if isinstance(widget, tk.Label):
